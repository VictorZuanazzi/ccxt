...
def load_balance_from_exchange(exchange):
    '''Fetches the balance in the account.
    
    Returns a of balances of the coins.
    '''
    try:
        ...
        print("Error:", error)
        wait()
        balances = load_balance_from_exchange(exchange)
    return balances
...
def load_balance(exchange, coin_of_interest = ''):
    ...
    if coin_of_interest == '':
        ...
        return_balance[coin_of_interest] = balances[free_balance][coin_of_interest]
    else:
...
def straight_sequence(one_trair, profit_coin):
    ...
    '''
    pair_path = []
    trade_coin = profit_coin
    for j in range(len(one_trair)):
        one_pair = parse_pair(one_trair[j])
...
def inverse_sequence(path):
    '''Returns the inverse trade path'''
    inverse_path = []
    for trade in path:
        if trade == 'bid':
            inverse_path.append('ask')
        else:
            inverse_path.append('bid')
    return inverse_path

def generate_sequence(trair, profit_coin):
    '''Defines the sequence of trades that must happen for each trair. 
    It computes the straight and the inverse sequences. Those are the only
    two possibilities for each trair.
    
    Return:
        trade_path = {['straight': [['bid','bid','ask'], 
                      'inverse': [['ask', 'ask', 'bid']],...]} list is the same 
    lenght as trair
    '''
    trade_path = []
    for i in range(len(trair)):
        trade_path.append({})
        trade_path[i]['straight'] = straight_sequence(trair[i], profit_coin[i])
        trade_path[i]['inverse'] = inverse_sequence(trade_path[i]['straight'])
...
def calculate_factor(ask, bid, trade_path):
    ...
    for i in range(len(trade_path)):
        if trade_path[i] == 'ask':
            unbalance_factor = unbalance_factor/ask[i][0]
        else:
            unbalance_factor = unbalance_factor*bid[i][0]
            
    return unbalance_factor

def calculate_volume(ask, bid, trade_path, pairs, profit_coin):
    ''' returns the max volumes possible for each trade.
    
    it looks too complicated for what it does. there must be a better way to do it.
    ...
    # transform all volumes into profit_coin volumes.
...
def find_unbalance(exchange, pairs, profit_coin, trade_path):
    ...
    unbalance_factor = {
            'straight': calculate_factor(ask, bid, trade_path['straight']), 
            'inverse': calculate_factor(ask, bid, trade_path['inverse'])}
...
#def define_volume(exchange, coin, profit_coin_volume, trade_volume):
#    '''returns the volume for the trade'''
#    
#    available = load_balance(exchange, coin)
#    
#    max_possible_volume = min(available, )
    
def min_volume(book_volume, available_volume, trair_volume = 0):
    if trair_volume == 0:
        trade_volume = min(book_volume, available_volume)
    else:
        trade_volume = min(book_volume, available_volume, trair_volume)
    return trade_volume

def market_buy(exchange, pair, volume):
    try:
        buy = exchange.create_market_buy_order(pair, volume) 
        print ('buy: ', buy)
    except (ccxt.RequestTimeout, 
            ccxt.ExchangeError, 
            ccxt.InvalidNonce) as error:
        print("Something went wrong when placing the order, new attempt will take place")
        print("In the mean while, check your internet connection.")
        print("Error:", error)
        market_buy(exchange, pair, volume)
    return buy

def limit_buy(exchange, pair, volume, price):

    try:
        buy = exchange.create_limit_buy_order(pair, volume, price) 
        print ('buy: ', buy)
    except (ccxt.RequestTimeout, 
            ccxt.ExchangeError, 
            ccxt.InvalidNonce) as error:
        ...
        print("In the mean while, check your internet connection.")
        print("Error:", error)
        limit_buy(exchange, pair, volume, price)
    return buy

def market_sell(exchange, pair, volume):
    try:
        sell = exchange.create_market_sell_order(pair, volume) 
        print ('sell: ', sell)
    except (ccxt.RequestTimeout, 
            ccxt.ExchangeError, 
            ccxt.InvalidNonce) as error:
        ...
        print("In the mean while, check your internet connection.")
        print("Error:", error)
        market_sell(exchange, pair, volume)
    return sell

def limit_sell(exchange, pair, volume, price):
    try:
        sell = exchange.create_limit_sell_order(pair, volume, price) 
        print ('sell: ', sell)
    except (ccxt.RequestTimeout, 
            ccxt.ExchangeError, 
            ccxt.InvalidNonce) as error:
        ...
        print("In the mean while, check your internet connection.")
        print("Error:", error)
        limit_sell(exchange, pair, volume,price)
    return sell
        
def get_unbalance(exchange, unbalance, trade_path, market_order):
    
    for i in range(len(unbalance['trair'])):
        trade = trade_path[unbalance['Sequence']][i]
        coin = parse_pair(unbalance['trair'][i])
        book_volume = unbalance[trade][i][1]
        trair_volume = 0
        
        if trade == 'ask':
            available_volume = load_balance(exchange, coin[1])[coin[1]]*unbalance['ask'][i][0]
            if i == 0:
                if coin[1] == unbalance['profit_coin']:
                    trair_volume = unbalance['Volume']*unbalance['ask'][i][0]
                elif trade_path[unbalance['Sequence']][2] == 'ask':
                    trair_volume = unbalance['Volume']*unbalance['ask'][2][0]
                else:
                    trair_volume = unbalance['Volume']*unbalance['bid'][2][0]
        else:
            available_volume = load_balance(exchange, coin[0])[coin[0]]*unbalance['bid'][i][0]
            if i == 0:
                if coin[0] == unbalance['profit_coin']:
                    trair_volume = unbalance['Volume']
                elif coin[1] == unbalance['profit_coin']:
                    trair_volume = unbalance['Volume']*unbalance['ask'][i][0]
                elif trade_path[unbalance['Sequence']][2] == 'ask':
                    trair_volume = unbalance['Volume']*unbalance['ask'][2][0]
                else:
                    trair_volume = unbalance['Volume']*unbalance['bid'][2][0]
        trade_volume = min_volume(book_volume, available_volume, trair_volume)
        if market_order == True:
            market_sell(exchange, unbalance['trair'][i], trade_volume) 
        else:
            limit_sell(exchange, unbalance['trair'][i], trade_volume, unbalance[trade][i][0]) 
        ...
        while True:
            for t in range(len(trair)):
                unbalance = find_unbalance(
                        ...
                        trade_path[t])
                if unbalance:
                    get_unbalance(exchange, unbalance, trade_path[t], market_order)
