...
def load_balance(exchange, coin_of_interest = ''):
    ...
    elif (coin_of_interest in balances[kucoin_key]) and type(coin_of_interest) == str:
...
def coin_mean_factor(oportunities, interesting_coin):
    '''Returns the real, weighted profit factor for each coin.
    ...
    for i in range(len(oportunities['profit_coin'])):
        volume[coin] += oportunities['Volume'][i]
...
def oportunities_stats(oportunities, interesting_coin):
    ...
    op['mean_real_factor'] = sum(oportunities['Unbalance factor'])/op['total'] - 3*FEE
    ...
    op['wighted_real_mean_factor'] = coin_mean_factor(oportunities, interesting_coin)
    #op['profit'] = op['total_volume']*(op['mean_real_factor'] -1)
    print_stats(op)
