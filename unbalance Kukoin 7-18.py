...
def get_unbalance(exchange, unbalance, trade_path):
    
    for i in range(len(unbalance['trair'])):
        trade = trade_path[unbalance['Sequence']][i]
        coin = parse_pair(unbalance['trair'][i])
        if trade == 'ask':
            book_volume = unbalance['ask'][1]
            available_volume = load_balance(exchange, coin[1])*unbalance['ask'][i][0]
            trair_volume = 0
            if i == 0:
            ...
            exchange.create_market_buy_order(unbalance['trair'][i], trade_volume)
        else:
            book_volume = unbalance['bid'][1]
            available_volume = load_balance(exchange, coin[0])*unbalance['bid'][i][0]
            trair_volume = 0
            if i == 0:
                if coin[0] == unbalance['profit_coin']:
                    ...
                    trair_volume = unbalance['Volume']*unbalance['bid'][2][0]
            trade_volume = min_volume(book_volume, available_volume, trair_volume)
            exchange.create_market_sell_order(unbalance['trair'][i], trade_volume) 
        ...
        while True:
            for t in range(len(trair)):
                unbalance = find_unbalance(
                        ...
                        trade_path[t])
                if unbalance:
                    #get_unbalance(exchange, unbalance, trade_path[t])
