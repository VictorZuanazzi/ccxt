...
def get_unbalance(exchange, unbalance, trade_path):
    ...
    volume = unbalance['Volume']
    if unbalance['Sequence'] == 'straight':
    
        for i in range(len(trade_path)):
            if trade_path[i] == 'ask':
                exchange.create_market_buy_order(unbalance['trair'][i], unbalance['Volume'])
            else:
                exchange.create_market_sell_order(unbalance['trair'][i], unbalance['Volume']) 
        balance = load_balance(exchange, unbalance['trair'][i])
