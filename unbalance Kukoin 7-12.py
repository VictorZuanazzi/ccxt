...
def print_oportunities(op):
    print('stats:')
    print(op)
    
def oportunities_stats(oportunities):
    op = {}
    op['total'] = len(oportunities['trair'])

    op['mean_factor'] = sum(oportunities['Unbalance factor'])/op['total']
    op['mean_real_factor'] = sum(oportunities['Unbalance factor'])/op['total'] - op['total']*3*FEE
    op['total_volume'] = sum(oportunities['Volume'])
    op['mean_volume'] = op['total_volume']/op['total']
        ...
        while True:
            for t in range(len(trair)):
                ...
                if unbalance:
                    ...
                    oportunities_stats(oportunities)
