...
def find_unbalance(exchange, pairs, profit_coin, trair_type):
...
def define_trair_type(trair, profit_coin):
    
    trair_type = []
    for i in range(len(trair)):
        profit_coin_position_start = trair[i][0].find(profit_coin[i])
        profit_coin_position_end = trair[i][2].find(profit_coin[i])
        
        if profit_coin_position_start > 1:
            if profit_coin_position_end < 1:
                #trair_type 1: ['xxx/profit_coin', 'xxx/yyy', 'profit_coin/yyy']
                trair_type.append(1)
            else:
                #trair_type: 2: ['xxx/profit_coin', 'xxx/yyy', 'yyy/profit_coin']
                trair_type.append(2)
        else:
            if profit_coin_position_end < 1:
                #trair_type 3: ['profit_coin/xxx', 'xxx/yyy', 'profit_coin/yyy']
                trair_type.append(3)
            else:
                #trair_type 4: ['profit_coin/xxx', 'xxx/yyy', 'yyy/profit_coin']
                #This is very unlikely to happen, but just in case.
                trair_type.append(4)
...
def main():
    ...
    trair = sort_pairs(trair, coins_of_interest)
    trair_type = define_trair_type(trair, coins_of_interest)
    ...
    try:
        while True:
            for t in range(len(trair)):
                unbalance = find_unbalance(exchange, trair[t], coins_of_interest[t], trair_type[t])
                if unbalance:
                    print('unbalance')
