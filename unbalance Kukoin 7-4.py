...
def load_order_book(exchange, pairs):
    ...
    try:
        ...
        return ask, bid
    except (ccxt.RequestTimeout, ccxt.ExchangeNotAvailable):
...
def calculate_volume(ask, bid, trade_path, mode = 'straight'):
    ''' NOT WORKING'''
    volume = [0,0,0]
    trade = trade_mode(mode)
...
def find_unbalance(exchange, pairs, profit_coin, trade_path):
    ...
    print('straight unbalance factor 0:', unbalance_factor['straight'],
          '\ninverse unbalance factor 1:', unbalance_factor['inverse'])
    ...
    try:
        while True:
            for t in range(len(trair)):
                unbalance = find_unbalance(exchange, trair[t], coins_of_interest[t], trade_path[t])
                if unbalance:
                    print('unbalance')
